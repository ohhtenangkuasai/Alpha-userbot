...
async def fastpurger(purg):
    ...
    if msgs:
        ...
        purg.chat_id, f"`Berhasil Menghapus Pesan Alpha`\
        \nJumlah Pesan Yang Dihapus {str(count)} Pesan")
    """
    if BOTLOG:
        await purg.client.send_message(
            BOTLOG_CHATID,
            "Berhasil Menghapus Pesan Alpha " + str(count) + " Pesan Berhasil  Dibersihkan.")
    """
    await sleep(2)
    await done.delete()
...
async def purgeme(delme):
    ...
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`Alpha Telah Menghapus Pesan,` " + str(count) + " Pesan Telah Dihapus`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
...
async def delete_it(delme):
    ...
    if delme.reply_to_msg_id:
        try:
            ...
            if BOTLOG:
                await delme.client.send_message(
                    BOTLOG_CHATID, "`Alpha Berhasil Menghapus Pesan`")
            """
