...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("`Terjadi Kesalahan... Mohon Lapor Ke Grup` @alphabot_support", str(err))
...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Alpha Ingin Mengaktifkan Perintah Global Banned!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Pengguna Ini`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Banned Akan Segera Aktif, Anda Akan Dibanned Secara Global Oleh Alpha`")
    ...
    if user:
        if user.id == 1682708454:
            return await dark.edit(
                f"`Anda Tidak Bisa Melakukan Global Banned Ke Mr.zeeone, Dia Adalah Pembuat Saya 😈`"
            )
        try:
        ...
        await dark.edit(f"`Mohon Balas Ke Pesan Sob`")
    try:
...
CMD_HELP.update(
    {
        ...
        \n  •  **Function : **Melakukan Banned Secara Global Ke Semua Grup Dimana Alpha Sebagai Admin.\
