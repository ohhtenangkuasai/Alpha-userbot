...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"•⎚• ⎆ __Alpha_ **Pong!**\n"
                    f"☞  __Ping:__ "
                    f"`%sms` \n"
                    f"☞  __Sisa Waktu:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    await pong.edit(f"- ALPHA -\n"
                    ...
                    f"**☞ i'm online  :** "
                    f"`{uptime}` \n"
                    f"__|━|⎆__ **My User  :** `{ALIVE_NAME}`" % (duration))
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("__ALPHA__")
    await pong.edit("__PONG__")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"┏━━| **ALPHA PONG** |━━卍\n"
                    f"┣|•  __Ping:__ "
                    f"`%sms` \n"
                    f"┗|• __Uptime:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**User** {ALIVE_NAME}\n"
                    f"**Pong!!** "
                    f"`%sms` \n"
                    f"**Uptime!!** "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    await pong.edit("⎚⎆ __Alpha__ **Test Ping!**\n`%sms`" % (duration))
